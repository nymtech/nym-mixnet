...
func main() {
	daemon.Start(func() {
		...
		flag.Duration("interval", 0, "Minimum interval between messages to be sent")
		flag.Int("num", 0, "Number of messages to send")
	},
		func() daemon.Service {
			...
			interval := flag.Lookup("interval").Value.(flag.Getter).Get().(time.Duration)
			numMessages := flag.Lookup("num").Value.(flag.Getter).Get().(int)
